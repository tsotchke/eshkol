...
The language descends from Scheme but compiles to native machine code through LLVM 21, targeting x86-64 and ARM64 architectures on macOS, Linux, and Windows. The v1.2.0-scale release ships model serialization via an extended ESKB binary format, a stable C FFI with pybind11 Python bindings and NumPy zero-copy interop, per-thread arenas for safe concurrent allocation, image I/O for vision pipelines (PNG/JPEG/BMP), CSV/DataFrame extensions with type inference, terminal plotting, JSON Schema Draft 7 validation, source-location-aware error messages, and a complete reproducibility artifact for the Self-Differentiating Neural Computer paper—a six-layer transformer with analytically-constructed weights that bit-identically simulates an 83-instruction bytecode VM with native reverse-mode automatic differentiation. Unusually for a compiled language, Eshkol preserves homoiconicity—Lisp's property of representing code as manipulable data structures. Compiled closures retain their symbolic S-expression forms, accessible at runtime for introspection and transformation. No other compiled language maintains this capability.
...
The v1.1-accelerate series, completed in April 2026, extended Eshkol from a CPU-only compiler to a heterogeneous computing system. Tensor operations dispatch through a four-tier hierarchy: SIMD micro-kernels for small tensors, cBLAS/Accelerate for medium workloads, Metal or CUDA for large, and XLA for very large or dynamic shapes. This is transparent to user code—the same tensor expression compiles to different backends based on operand size and available hardware. Parallel primitives (`parallel-map`, `parallel-fold`, `future`/`force`) exploit multi-core CPUs via a work-stealing thread pool. v1.2-scale builds on this foundation by closing the deployment loop: trained weights and knowledge bases serialize to disk, Python interop is first-class, and the build pipeline produces clean binaries across the full 16-lane CI matrix (Linux/macOS/Windows × x64/ARM64 × lite/XLA/CUDA).
