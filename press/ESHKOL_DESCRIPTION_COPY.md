...
The language descends from Scheme but compiles to native machine code through LLVM 21, targeting x86-64 and ARM64 architectures on macOS, Linux, and Windows. The v1.1.13-accelerate release adds native Windows ARM64 support, rewrites the release workflow into a 16-lane build matrix producing lite/XLA/CUDA variants for every supported platform, fixes critical bytecode-VM closure bugs that affected the browser REPL, and hardens setjmp/longjmp on Windows for both x64 and ARM64. Unusually for a compiled language, Eshkol preserves homoiconicity—Lisp's property of representing code as manipulable data structures. Compiled closures retain their symbolic S-expression forms, accessible at runtime for introspection and transformation. No other compiled language maintains this capability.
