...
/**
 * Compute a row-major linear offset into a tensor from a SINGLE tagged
 * argument that may be either a scalar or a cons-list of indices.
 *
 * Bug II (2026-05-07): tensor-set!/tensor-ref invoked with the list
 * idiom `(tensor-set! t (list i j k) v)` previously routed through
 * `eshkol_unwrap_list_index`, which returns just the car of the list.
 * That collapsed every multi-dim index `(0 _ _)` to flat[0] — the
 * symptom Noesis caught was a single 3D write hitting flat[0..4]
 * (writes went to flat[0] only; reads of (0,0,0)..(0,1,1) all read
 * back flat[0] because `car` of each index list is 0). The real fix
 * is to walk the cons chain and compute strides from the tensor's
 * own dims, not to extract one element.
 *
 * Behavior:
 *   - tv is HEAP_PTR + CONS: walk up to `ndim` cells (or until the
 *     list ends), treat each car as a dimensional index, compute the
 *     row-major offset using `dims`. Indices past the end of the list
 *     default to 0 — this matches NumPy's "fewer indices than dims"
 *     convention (selects the [0] hyperplane on trailing dims, which
 *     is rarely what callers want, so combine with a bounds check at
 *     the call site if you care).
 *   - tv is INT64/DOUBLE: scalar 1-D index, return as int64. Useful
 *     for the (tensor-ref t i) shape against a flat 1-D tensor.
 *   - anything else: fall back to interpreting tv.data as int64
 *     (back-compat with `eshkol_unwrap_list_index`'s relaxed contract).
 *
 * `ndim` and `dims` come from the tensor metadata at the call site —
 * codegen loads them once and passes both into this helper, so the
 * runtime walks the list exactly once and yields the final offset.
 *
 * Capped at 16 dimensions to avoid heap allocation. Tensors beyond
 * 16-D are vanishingly rare (the runtime model only allocates `dims`
 * for as many as the user asks for) — bump the bound if that ever
 * stops being true.
 */
int64_t eshkol_tensor_linear_from_index_arg(
    ...
    if (base_type == ESHKOL_VALUE_HEAP_PTR && tv.data.ptr_val != 0) {
        ...
        if (hdr->subtype == HEAP_SUBTYPE_CONS) {
            ...
            // Single-pass Horner-style accumulation — handles arbitrary
            // tensor rank without a pre-sized stack buffer. The row-major
            // linear offset
            //   linear = i_0 * (d_1 * d_2 * ... * d_{n-1})
            //          + i_1 * (d_2 * ... * d_{n-1})
            //          + ...
            //          + i_{n-1} * 1
            // factors as the recurrence
            //   linear_0 = i_0
            //   linear_k = linear_{k-1} * d_k + i_k
            // so we can walk the cons list left-to-right, multiplying
            // by dims[k] for k>=1 at each step. For partial indexing
            // (count<ndim) we then "shift" by multiplying through the
            // unindexed trailing dims so the result lands at the start
            // of the slice. No buffer, no MAX_NDIM cap.
            int64_t linear = 0;
            int64_t count = 0;
            ...
            while (true) {
                uint8_t cb = cur.type;
                if (cb < 8) cb &= 0x0F;
                if (cb != ESHKOL_VALUE_HEAP_PTR || cur.data.ptr_val == 0) break;
                const eshkol_object_header_t* h =
                    ESHKOL_GET_HEADER((void*)cur.data.ptr_val);
                if (h->subtype != HEAP_SUBTYPE_CONS) break;
                if (count >= ndim) break; /* over-indexed; stop walking */

                eshkol_tagged_value_t car = arena_tagged_cons_get_tagged_value(
                    (const void*)cur.data.ptr_val, false);
                eshkol_tagged_value_t cdr = arena_tagged_cons_get_tagged_value(
                    (const void*)cur.data.ptr_val, true);

                uint8_t carb = car.type;
                if (carb < 8) carb &= 0x0F;
                int64_t v;
                if (carb == ESHKOL_VALUE_INT64) {
                    v = (int64_t)car.data.int_val;
                } else if (carb == ESHKOL_VALUE_DOUBLE) {
                    double d;
                    memcpy(&d, &car.data, sizeof(double));
                    v = (int64_t)d;
                } else {
                ...
                if (count == 0) {
                    ...
                    linear = linear * dims[count] + v;
                }
                count++;
                cur = cdr;
            }

            /* Partial indexing: shift to slice start by multiplying
             * through the unindexed trailing dims. */
...
/**
 * vref-specific: unwrap an index argument into a flat offset, taking
 * the tensor's own ndim/dims into account when the index is a cons
 * list. Used by the 2-arg `(tensor-ref t idx)` codegen path at
 * llvm_codegen.cpp ~line 22748, which previously used
 * `eshkol_unwrap_list_index` and silently collapsed `(list i j k)`
 * to `flat[i]` for any multi-dim tensor (Bug II).
 *
 * Behavior:
 *   - vec_tv is a tensor (HEAP_PTR + HEAP_SUBTYPE_TENSOR), idx is a
 *     cons list → walk the list, compute row-major linear offset
 *     using the tensor's own dims (matches NumPy `t[(i,j,k)]`).
 *   - anything else → fall back to `eshkol_unwrap_list_index`
 *     (preserves current behavior for scheme-vector inputs and
 *     scalar inputs to scalar `gradient` returns).
 *
 * Done as a runtime call (rather than reshaping the codegen path) so
 * we don't have to rewire the type-dispatch on vec_tv, which happens
 * AFTER the index is unwrapped today. The helper does its own runtime
 * type peek and DTRT in either case.
 */
int64_t eshkol_vref_unwrap_index(
    ...
    eshkol_tagged_value_t vec = *vec_tv_in;
    uint8_t vbase = vec.type;
    if (vbase < 8) vbase &= 0x0F;

    if (vbase == ESHKOL_VALUE_HEAP_PTR && vec.data.ptr_val != 0) {
        const eshkol_object_header_t* vhdr =
            ESHKOL_GET_HEADER((void*)vec.data.ptr_val);
        if (vhdr->subtype == HEAP_SUBTYPE_TENSOR) {
            /* Inline the eshkol_tensor layout (lib/core/arena_memory.h:397):
             *   { uint64_t* dimensions; uint64_t num_dimensions;
             *     int64_t*  elements;   uint64_t total_elements; }
             * Including arena_memory.h directly conflicts with the
             * function declarations runtime.cpp already locally externs,
             * so mirror just the two fields we need here. Static-asserted
             * layout in arena_memory.h keeps this stable. */
            struct vref_tensor_layout {
                uint64_t* dimensions;
                uint64_t  num_dimensions;
                int64_t*  elements;
                uint64_t  total_elements;
            };
            const vref_tensor_layout* t =
                (const vref_tensor_layout*)(uintptr_t)vec.data.ptr_val;
            return eshkol_tensor_linear_from_index_arg(
                idx_tv_in,
                (const int64_t*)t->dimensions,
                (int64_t)t->num_dimensions);
        }
    }
...
/**
 * Companion to `eshkol_tensor_linear_from_index_arg` — counts how many
 * dimensional indices a single argument supplies. Returns the cons
 * length for a list, 1 for a scalar.
 *
 * Used by tensor-ref codegen so the scalar-vs-slice decision uses the
 * right "is num_indices == ndim?" predicate. Walks the cons chain
 * to its end with no fixed cap, so arbitrary tensor rank is supported.
 */
    ...
    if (base_type == ESHKOL_VALUE_HEAP_PTR && tv.data.ptr_val != 0) {
        ...
        if (hdr->subtype == HEAP_SUBTYPE_CONS) {
            ...
            while (true) {
                ...
                if (h->subtype != HEAP_SUBTYPE_CONS) break;
                count++;
                cur = arena_tagged_cons_get_tagged_value(
                    (const void*)cur.data.ptr_val, true);
            }
...
/* ─────────────────────────────────────────────────────────────────────
 * Native pixel fills — answer to Noesis's 2026-05-07 request for
 *   (tensor-rect-fill!) and (tensor-disk-fill!)
 * primitives so per-pixel game rendering doesn't have to round-trip
 * through `tensor-set!` for every cell.
 *
 * Both helpers operate directly on the raw `int64_t* elements`
 * field of an eshkol_tensor_t, treating each cell as a double bit-
 * pattern (the storage convention for tensors). The codegen passes a
 * stack-allocated `double channels[N]` buffer + `num_channels` so the
 * runtime doesn't have to walk a cons list per pixel.
 *
 * Tensor-shape model:
 *   - 2D tensor (H, W):   num_channels must be 1; one fill value per cell.
 *   - 3D tensor (H, W, C): num_channels can be 1 (broadcast to all C
 *     channels — cheap "fill grayscale on RGB"), or exactly C (per-
 *     channel write — common RGB / RGBA case).
 *   - higher-rank tensors are not supported by these primitives —
 *     callers should use multi-arg (tensor-set! ...) directly.
 *
 * Bounds:
 *   - rectangles are clamped to [0, dim) on each axis; out-of-range
 *     corners produce no writes (no error).
 *   - disks are clamped at the bounding box; pixels outside the
 *     tensor are skipped.
 *
 * The helpers themselves are pure C — no allocation, no exceptions.
 * They scale linearly with pixel count and avoid the per-cell
 * tagged-dispatch + cons-walk overhead of the per-set!  loop.
 * ───────────────────────────────────────────────────────────────────*/
static inline void* eshkol_tensor_payload(
    ...
    *out_h = 0; *out_w = 0; *out_c = 0;
    if (!t_tv) return NULL;
    eshkol_tagged_value_t v = *t_tv;
    uint8_t base = v.type;
    if (base < 8) base &= 0x0F;
    if (base != ESHKOL_VALUE_HEAP_PTR || v.data.ptr_val == 0) return NULL;
    const eshkol_object_header_t* hdr = ESHKOL_GET_HEADER((void*)v.data.ptr_val);
    if (hdr->subtype != HEAP_SUBTYPE_TENSOR) return NULL;
    /* Mirror the layout from arena_memory.h:397 — must stay in sync
     * (sizeof asserted to 32 there). */
    ...
    const fill_tensor_layout* t = (const fill_tensor_layout*)(uintptr_t)v.data.ptr_val;
    if (t->num_dimensions == 2) {
        *out_h = (int64_t)t->dimensions[0];
        *out_w = (int64_t)t->dimensions[1];
        *out_c = 1;
    } else if (t->num_dimensions == 3) {
        *out_h = (int64_t)t->dimensions[0];
        *out_w = (int64_t)t->dimensions[1];
        *out_c = (int64_t)t->dimensions[2];
    } else {
        return NULL; /* unsupported rank */
    }
    return t->elements;
}

static inline void eshkol_tensor_write_pixel(
    int64_t* elems, int64_t y, int64_t x,
    int64_t W, int64_t C,
    const double* channels, int64_t num_channels) {
    int64_t base = (y * W + x) * C;
    if (num_channels == 1 && C > 1) {
        /* Broadcast scalar to all channels. */
        int64_t bits;
        memcpy(&bits, &channels[0], sizeof(double));
        for (int64_t c = 0; c < C; c++) elems[base + c] = bits;
    } else {
        int64_t writes = (num_channels < C) ? num_channels : C;
        for (int64_t c = 0; c < writes; c++) {
            int64_t bits;
            memcpy(&bits, &channels[c], sizeof(double));
            elems[base + c] = bits;
        }
    }
}
...
void eshkol_tensor_rect_fill(
    ...
    const double* channels, int64_t num_channels) {
    int64_t H, W, C;
    int64_t* elems = (int64_t*)eshkol_tensor_payload(t_tv, &H, &W, &C);
    if (!elems) return;

    /* Clamp to tensor bounds; allow inverted corners (caller error → no-op). */
    if (row0 > row1) { int64_t t = row0; row0 = row1; row1 = t; }
    if (col0 > col1) { int64_t t = col0; col0 = col1; col1 = t; }
    if (row0 < 0) row0 = 0;
    if (col0 < 0) col0 = 0;
    if (row1 > H) row1 = H;
    if (col1 > W) col1 = W;
    ...
    for (int64_t y = row0; y < row1; y++) {
        for (int64_t x = col0; x < col1; x++) {
            eshkol_tensor_write_pixel(elems, y, x, W, C, channels, num_channels);
        }
    }
}
...
void eshkol_tensor_disk_fill(
    ...
    int64_t y0 = cy - radius, y1 = cy + radius + 1;
    int64_t x0 = cx - radius, x1 = cx + radius + 1;
    if (y0 < 0) y0 = 0; if (x0 < 0) x0 = 0;
    if (y1 > H) y1 = H; if (x1 > W) x1 = W;
