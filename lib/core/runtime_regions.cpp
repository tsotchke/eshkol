...
static EvacKind evac_kind_for(EvacState& st, const eshkol_tagged_value_t& v, const void* old_data) {
    ...
    if (ESHKOL_IS_ANY_CALLABLE_TYPE(type)) {
        ...
        if (sub == CALLABLE_SUBTYPE_AD_NODE) {
            ...
            // SW-164: in ARENA-SPAN mode, ANY AD node is a refusal — inputs or
            // not. The tape's nodes[] array holds the original pointer, so
            // moving the node breaks the recorded evaluation order even when
            // the node itself copies completely.
            st.refuse();
            if (!st.span_arena &&
                (n->input1 || n->input2 || n->input3 || n->input4 ||
