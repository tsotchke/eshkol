...
static int vm_dual_has_variation(const VmDual* d) {
    ...
    double* channels[4] = {d->coeff, d->tangent_coeff, d->tangent2_coeff, d->mixed_coeff};
    VmRational** exact[4] = {d->exact_coeff, d->exact_tangent_coeff,
                            d->exact_tangent2_coeff, d->exact_mixed_coeff};
    for (int channel = 0; channel < 4; ++channel)
        for (uint32_t k = channel == 0 ? 1u : 0u; k <= d->order; ++k)
            if ((channels[channel] && channels[channel][k] != 0.0) ||
                (exact[channel] && exact[channel][k] &&
                 !vm_rational_is_zero(exact[channel][k]))) return 1;
    return 0;
}
