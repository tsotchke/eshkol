...
static VmComplex* vm_complex_new_d(VmRegionStack* rs, double real, double imag,
    ...
    if (z) { z->dreal = dreal; z->dimag = dimag; }
    return z;
}

/** Result v of a holomorphic function at z, with derivative fp = f'(z):
 *  tangent = fp * dz. */
static VmComplex* vm_cpx_out_holo(VmRegionStack* rs, eshkol_cpx v, eshkol_cpx fp, const VmComplex* z) {
...
static VmComplex* vm_complex_conjugate(VmRegionStack* rs, const VmComplex* z) {
    return vm_complex_new_d(rs, z->real, -z->imag, z->dreal, -z->dimag);
}
...
static VmComplex* vm_complex_add(VmRegionStack* rs, const VmComplex* a, const VmComplex* b) {
    return vm_complex_new_d(rs, a->real + b->real, a->imag + b->imag,
...
static VmComplex* vm_complex_sqrt(VmRegionStack* rs, const VmComplex* z) {
    ...
    return vm_cpx_out_holo(rs, v, eshkol_cpx_div(eshkol_cpx_make(1.0, 0.0), eshkol_cpx_scale(v, 2.0)), z);
}
...
static VmComplex* vm_complex_exp(VmRegionStack* rs, const VmComplex* z) {
    ...
    if (!vm_complex_has_tangent(z)) return vm_cpx_out(rs, v);
    return vm_cpx_out_holo(rs, v, v, z);
}
...
static VmComplex* vm_complex_log(VmRegionStack* rs, const VmComplex* z) {
    ...
    return vm_cpx_out_holo(rs, v, eshkol_cpx_div(eshkol_cpx_make(1.0, 0.0), zi), z);
}
...
static VmComplex* vm_complex_sin(VmRegionStack* rs, const VmComplex* z) {
    ...
    if (!vm_complex_has_tangent(z)) return vm_cpx_out(rs, v);
    return vm_cpx_out_holo(rs, v, eshkol_cpx_cos(zi), z);
}
...
static VmComplex* vm_complex_cos(VmRegionStack* rs, const VmComplex* z) {
    ...
    if (!vm_complex_has_tangent(z)) return vm_cpx_out(rs, v);
    return vm_cpx_out_holo(rs, v, eshkol_cpx_scale(eshkol_cpx_sin(zi), -1.0), z);
}
...
static VmComplex* vm_complex_tan(VmRegionStack* rs, const VmComplex* z) {
    ...
    if (!vm_complex_has_tangent(z)) return vm_cpx_out(rs, v);
    return vm_cpx_out_holo(rs, v, eshkol_cpx_add(eshkol_cpx_make(1.0, 0.0), eshkol_cpx_mul(v, v)), z);
}
...
static VmComplex* vm_complex_asin(VmRegionStack* rs, const VmComplex* z) {
    ...
    if (!vm_complex_has_tangent(z)) return vm_cpx_out(rs, v);
    return vm_cpx_out_holo(rs, v, eshkol_cpx_div(eshkol_cpx_make(1.0, 0.0), eshkol_cpx_sqrt(eshkol_cpx_sub(eshkol_cpx_make(1.0, 0.0), eshkol_cpx_mul(zi, zi)))), z);
}
...
static VmComplex* vm_complex_atan(VmRegionStack* rs, const VmComplex* z) {
    ...
    if (!vm_complex_has_tangent(z)) return vm_cpx_out(rs, v);
    return vm_cpx_out_holo(rs, v, eshkol_cpx_div(eshkol_cpx_make(1.0, 0.0), eshkol_cpx_add(eshkol_cpx_make(1.0, 0.0), eshkol_cpx_mul(zi, zi))), z);
}
...
static VmComplex* vm_complex_sinh(VmRegionStack* rs, const VmComplex* z) {
    ...
    if (!vm_complex_has_tangent(z)) return vm_cpx_out(rs, v);
    return vm_cpx_out_holo(rs, v, eshkol_cpx_cosh(zi), z);
}
...
static VmComplex* vm_complex_cosh(VmRegionStack* rs, const VmComplex* z) {
    ...
    if (!vm_complex_has_tangent(z)) return vm_cpx_out(rs, v);
    return vm_cpx_out_holo(rs, v, eshkol_cpx_sinh(zi), z);
}
...
static VmComplex* vm_complex_tanh(VmRegionStack* rs, const VmComplex* z) {
    ...
    if (!vm_complex_has_tangent(z)) return vm_cpx_out(rs, v);
    return vm_cpx_out_holo(rs, v, eshkol_cpx_sub(eshkol_cpx_make(1.0, 0.0), eshkol_cpx_mul(v, v)), z);
}
