 ...
 *         is not required because continuations restore control, not the
 *         mutable location (SW-62). */
...
static void compile_quasiquote(FuncChunk* c, Node* node) {
    ...
    if (node->type == N_SYMBOL) {
        ...
        for (int p = 0; p < n_packs; p++) {
            uint64_t pack = 0;
            for (int b = 0; b < 8 && p * 8 + b < len; b++)
                pack |= ((uint64_t)(unsigned char)node->symbol[p * 8 + b]) << (b * 8);
            chunk_emit(c, OP_CONST, chunk_add_const(c, INT_VAL((int64_t)pack)));
        }
        chunk_emit(c, OP_NATIVE_CALL, 101);
        return;
...
static void compile_expr_impl(FuncChunk* c, Node* node, int tail) {
    ...
    if (is_sym(head, "cross-entropy-loss") && node->n_children == 3) {
        compile_expr(c, node->children[1], 0);
        compile_expr(c, node->children[2], 0);
        chunk_emit(c, OP_NATIVE_CALL, 480);
        return;
    }
...
static const BuiltinDef BUILTINS[] = {
    ...
    {"mse-loss", 459, 2}, {"cross-entropy-loss", 480, 2},
    /* AD ops (missing) */
