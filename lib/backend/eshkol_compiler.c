...
static Node* make_node(NodeType t) { Node* n = (Node*)calloc(1, sizeof(Node)); n->type = t; return n; }
static void add_child(Node* p, Node* c) { p->children = (Node**)realloc(p->children, (p->n_children+1)*sizeof(Node*)); p->children[p->n_children++] = c; }
...
static Node* parse_sexp(void) {
    ...
    if (*src_ptr == '\'') {
        src_ptr++;
        Node* q = make_node(N_LIST);
        Node* qs = make_node(N_SYMBOL); strcpy(qs->symbol, "quote");
        add_child(q, qs);
    ...
    if (*src_ptr == '"') {
        ...
        strncpy(n->symbol, buf, 127);
        return n;
    }
    if (*src_ptr == '#') {
        if (src_ptr[1] == 't' && (src_ptr[2] == 0 || isspace(src_ptr[2]) || src_ptr[2] == ')')) {
            src_ptr += 2; Node* n = make_node(N_BOOL); n->numval = 1; strcpy(n->symbol, "#t"); return n;
        }
        if (src_ptr[1] == 'f' && (src_ptr[2] == 0 || isspace(src_ptr[2]) || src_ptr[2] == ')')) {
            src_ptr += 2; Node* n = make_node(N_BOOL); n->numval = 0; strcpy(n->symbol, "#f"); return n;
        }
        ...
        if (src_ptr[1] == '(') {
            src_ptr += 2; /* skip #( */
            Node* vec = make_node(N_LIST);
            Node* tag = make_node(N_SYMBOL); strcpy(tag->symbol, "vector");
            add_child(vec, tag);
    ...
    if (isdigit(*src_ptr) || (*src_ptr == '-' && isdigit(src_ptr[1]))) {
        ...
        while (isdigit(*src_ptr) || *src_ptr == '.') buf[i++] = *src_ptr++;
        buf[i] = 0;
        Node* n = make_node(N_NUMBER); n->numval = atof(buf); return n;
    }
    /* Symbol */
    char buf[128]; int i = 0;
    while (*src_ptr && !isspace(*src_ptr) && *src_ptr != '(' && *src_ptr != ')' && *src_ptr != '"')
        buf[i++] = *src_ptr++;
    buf[i] = 0;
    Node* n = make_node(N_SYMBOL); strncpy(n->symbol, buf, 127); return n;
}
...
static int chunk_add_const(FuncChunk* c, Value v) {
    ...
    if (c->n_constants >= MAX_CONSTS) return 0;
...
static int add_local(FuncChunk* c, const char* name) {
    if (c->n_locals >= MAX_LOCALS) return -1;
    int slot = c->n_locals;
...
static void compile_expr(FuncChunk* c, Node* node, int tail) {
    ...
    if (is_sym(head, "make-vector") && node->n_children >= 2) {
        ...
        /* TODO: full make-vector via native */
        chunk_emit(c, OP_VEC_CREATE, 1); /* placeholder */
        return;
    }
    ...
    /* (require module) — no-op for VM (all builtins are always available) */
    if (is_sym(head, "require")) {
        chunk_emit(c, OP_NIL, 0);
        return;
    }

    /* (provide name ...) — no-op for VM */
    ...
    if (is_sym(head, "apply") && node->n_children == 3) {
        /* TODO: proper apply needs to unpack the list onto the stack */
        /* For now: handle simple case (apply f (list a b c)) */
        compile_expr(c, node->children[1], 0); /* push f */
        compile_expr(c, node->children[2], 0); /* push args list */
        /* Unpack list manually? This requires knowing the length at compile time.
         * For dynamic lists: need a runtime unpack loop. Defer to NATIVE_CALL. */
...
static void print_value(Value v, HeapObject* heap, int depth);
...
static void print_value(Value v, HeapObject* heap, int depth) {
    ...
    case VAL_STRING: printf("\"%.*s\"", heap[v.as.ptr].string.len, heap[v.as.ptr].string.data); break;
    case VAL_PAIR: {
        printf("(");
        Value cur = v; int first = 1;
        while (cur.type == VAL_PAIR && depth < 50) {
            if (!first) printf(" "); first = 0;
            print_value(heap[cur.as.ptr].cons.car, heap, depth + 1);
            cur = heap[cur.as.ptr].cons.cdr;
        }
        if (cur.type != VAL_NIL) { printf(" . "); print_value(cur, heap, depth + 1); }
        printf(")");
        break;
    }
    case VAL_VECTOR: {
        ...
        for (int i = 0; i < heap[v.as.ptr].vector.len; i++) {
            if (i > 0) printf(" ");
            print_value(heap[v.as.ptr].vector.items[i], heap, depth + 1);
        }
        printf(")");
        break;
    }
...
static void execute_chunk(FuncChunk* chunk) {
    ...
    #define PUSH(v) do { if(sp>=STACK_SIZE){printf("STACK OVERFLOW\n");error=1;}else stack[sp++]=(v); } while(0)
    #define POP() (stack[--sp])
    #define PEEK(off) (stack[sp-1-(off)])
    ...
    int max_depth = 0;
    int trace_on = 0; /* disabled */
    while (!halted && !error && pc < chunk->code_len) {
        if (frame_count > max_depth) max_depth = frame_count;
        if (++insn_count > 10000000LL) { printf("RUNAWAY (10M insns, depth=%d, heap=%d)\n", max_depth, heap_next); error=1; break; }
        ...
        case OP_MOD: {
            ...
            } else { PUSH(NUM_VAL(fmod(AS_NUM(a),bv))); }
            break;
        }
        ...
        case OP_GET_LOCAL: PUSH(stack[fp + ins.operand]); break;
        case OP_SET_LOCAL: stack[fp + ins.operand] = PEEK(0); sp--; break;
        case OP_GET_UPVALUE: {
            Value cl = stack[fp - 1];
        ...
        case OP_JUMP: pc = ins.operand; break;
        case OP_JUMP_IF_FALSE: { Value v=POP(); if(IS_FALSY(v)) pc=ins.operand; break; }
        case OP_LOOP: pc = ins.operand; break;
        ...
        case OP_NATIVE_CALL: {
            ...
            case 53: { /* build-string: stack has [len, char0, char1, ...charN-1] at bottom */
                /* Pop chars in reverse, then len */
                int len_idx = sp - 1;
                /* Find the length value (it was pushed first, so it's deepest) */
                /* Actually: constants pushed in order: len, c0, c1, ... cN-1
                 * Stack: [..., len, c0, c1, ..., cN-1]. N = len value.
                 * We need to read len first, then pop chars. */
                /* Read the top values: the chars are on top, len is below */
                /* Find len by looking at how many items we need */
                /* Hmm, this is tricky without knowing N. Use operand encoding instead. */
                /* Simpler: NATIVE_CALL 53 with operand = string length.
                 * But NATIVE_CALL only has one operand (the function ID).
                 * Alternative: the len is TOS-N, chars are TOS-N+1..TOS.
                 * Pop all chars into buffer, then pop len. */
                /* For now: the len is below the chars on the stack.
                 * Pop chars in reverse into a buffer, pop len, create string. */
                /* We need to know how many chars. Read len from deep in stack. */
                /* Actually, let's use a simpler approach: push len, then use it. */
                /* The stack has: ..., len_val, c0, c1, ..., cN-1 where N = len_val */
                /* But we don't know N at this point in the VM! */
                /* Fix: just allocate a small string and use a different calling convention. */
                /* Simplest: don't push individual chars. Instead, encode the string
                 * data directly in the constant pool as a special value. */
                /* FALLBACK: create empty string for now */
                int32_t sptr = HALLOC(); if (sptr < 0) break;
                heap[sptr].type = HEAP_STRING;
                heap[sptr].string.len = 0;
                heap[sptr].string.data[0] = 0;
            ...
            case 185: { break; } /* string->symbol (no-op) */
            ...
            /* take, drop, any, every, find, sort — implemented as scheme prelude below */
            case 225: case 226: case 227: case 228: case 229: case 230:
                printf("NATIVE_CALL %d: higher-order list op not yet implemented as native\n", fid);
                PUSH(((Value){.type=VAL_NIL})); break;
            ...
            case 240: { /* display (as native, for first-class use) */
                Value v=POP(); print_value(v, heap, 0); PUSH(((Value){.type=VAL_NIL})); break;
            }
            case 241: { /* write (same as display for now) */
                Value v=POP(); print_value(v, heap, 0); PUSH(((Value){.type=VAL_NIL})); break;
            }
            ...
            case 132: { /* force: force a promise #(forced? thunk-or-value) */
                ...
                if (forced.type == VAL_BOOL && forced.as.b) {
                    ...
                    if (thunk.type == VAL_CLOSURE) {
                        ...
                        /* When RETURN executes, result will be on stack.
                         * We can't memoize here — need a post-force hook.
                         * For now: evaluate but don't memoize (correct but inefficient).
                         * TODO: memoization requires wrapping the return. */
                    } else {
                        /* Thunk is already a value — return it */
                        PUSH(thunk);
                    }
                }
                break;
            ...
            default:
                printf("NATIVE_CALL %d not implemented\n", fid);
                PUSH(((Value){.type=VAL_NIL}));
                break;
            }
            break;
...
static void compile_and_run(const char* source) {
    ...
    static const char* scheme_prelude =
        ...
        "(define (filter pred lst) (if (null? lst) (list) (if (pred (car lst)) (cons (car lst) (filter pred (cdr lst))) (filter pred (cdr lst)))))\n"
