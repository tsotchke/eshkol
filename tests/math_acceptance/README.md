...
|---|---|---|---|
| `exact_rational_memory_test.esk` | SW-164, SW-165 | the exact harmonic sum H_5000 completes with the right denominator size and prints no heap-limit diagnostic | open: the arithmetic is right and RESULT: ALL PASS is printed, but 139,224 heap-limit diagnostic lines come with it; inverted | (closed 2026-09-11 with the exact-tower wave: prints RESULT: ALL PASS with no heap-limit diagnostic on the integrated build; no longer inverted)
