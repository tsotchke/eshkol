...
As of this suite's merge onto the v1.3.5 candidate, three of the six entries
below have closed and their `WILL_FAIL` inversions are gone (the CMake block
names them individually rather than inverting the whole suite). The remaining
three still fail on the candidate and are still inverted; the measured state
of each is recorded in the `status` column, taken from a run of every program
under `-r` against both this tree and the candidate's own build.

| test | ledger | what must hold | status |
|---|---|---|---|
| `exact_rational_memory_test.esk` | SW-164, SW-165 | the exact harmonic sum H_5000 completes with the right denominator size and prints no heap-limit diagnostic | closed: prints RESULT: ALL PASS, not inverted |
...
| `nursery_cond_test_shape_test.esk` | (filed 2026-09-10: nursery defeated by a `cond` clause with an allocating test) | a loop whose tail call sits under a `cond` test that allocates reclaims per iteration, printing no heap-limit diagnostic | closed: prints RESULT: ALL PASS, not inverted |
