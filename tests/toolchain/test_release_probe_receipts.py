...
'''
    ...
    def test_release_prepares_all_receipts_before_architecture_verification(self):
        workflow = (ROOT / ".github/workflows/release.yml").read_text()
        begin = workflow.index("      - name: Emit evidence traces")
        vm = workflow.index("scripts/run_vm_parity.sh", begin)
        abi = workflow.index("bash scripts/run_release_invariant_probes.sh", begin)
        grade = workflow.index("      - name: ICC architecture-verify", begin)
        self.assertLess(vm, abi)
        self.assertLess(abi, grade)
