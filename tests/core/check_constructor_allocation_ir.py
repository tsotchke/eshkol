...
NAME = r"[-a-zA-Z$._0-9]+"
ALLOCATORS = r"arena_allocate_(?:vector_with_header|cons_with_header|cons_cell|closure_with_header)"
    ...
    for function in re.findall(r"^define .*?^}", text, re.M | re.S):
        ...
        for name, lines in blocks.items():
            ...
            for call in re.finditer(r"(%" + NAME + r") = call ptr @" + ALLOCATORS + r"\([^\n]*", body):
                ...
                failure_body = "\n".join(blocks[failure])
                assert "call void @eshkol_raise_allocation_failure(" in failure_body
