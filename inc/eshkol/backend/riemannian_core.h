...
static void eshkol_rm_gyration(const double* a, const double* b, const double* w,
                               double ball_parameter, int n, double* out)
{
    const double ab = eshkol_rm_dot(a, b, n);
    const double a2 = eshkol_rm_dot(a, a, n);
    const double b2 = eshkol_rm_dot(b, b, n);
    ...
    const double denominator = eshkol_rm_mobius_den(a, b, ball_parameter, n);
    const double A = -ball_parameter * ball_parameter * aw * b2 +
                     ball_parameter * bw +
                     2.0 * ball_parameter * ball_parameter * ab * bw;
    const double C = -ball_parameter * ball_parameter * bw * a2 -
                     ball_parameter * aw;
    for (int i = 0; i < n; ++i) {
