...
The squared Riemannian norm of tangent vector `v` at `x.` Used by the intrinsic Adam second moment, which must be a scalar in the manifold's own metric rather than a per-coordinate quantity.
...
The Gram determinant |x|^2|y|^2 - <x,y>^2 by LAGRANGE'S IDENTITY, sum_{i<j} (x_i y_j - x_j y_i)^2 -- a sum of squares, so it has no cancellation and is exactly zero for collinear vectors.
...
| Symbol | Kind | Line |
|---|---|---:|
| `ESHKOL_RM_LAMBDA0` | Macro | 100 |
| `ESHKOL_RM_FLAT_LAMBDA` | Macro | 106 |
| `ESHKOL_RM_SPHERE_TOL` | Macro | 112 |
| `ESHKOL_RM_TANGENT_TOL` | Macro | 120 |
| `ESHKOL_RM_PSI_SMALL` | Macro | 125 |
| `ESHKOL_RM_TAU_SMALL` | Macro | 128 |
| `ESHKOL_RM_GRAM_EXACT_DIM` | Macro | 134 |
| `eshkol_rm_dot` | Function | 136 |
| `eshkol_rm_norm` | Function | 142 |
| `eshkol_rm_points_equal` | Function | 151 |
| `eshkol_rm_dyadic_t` | Struct | 162 |
| `eshkol_rm_dyadic_from_double` | Function | 167 |
| `eshkol_rm_all_finite` | Function | 193 |
| `eshkol_rm_gcd_u64` | Function | 200 |
| `eshkol_rm_cross_ratio_equal` | Function | 213 |
| `eshkol_rm_sphere_antipodal` | Function | 242 |
| `eshkol_rm_sphere_log_map_stable` | Function | 271 |
| `eshkol_rm_sphere_angle` | Function | 327 |
