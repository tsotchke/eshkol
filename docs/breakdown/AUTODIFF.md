...
|-----------|------|-------|---------|
| **AD Codegen** | [`lib/backend/autodiff_codegen.cpp`](../../lib/backend/autodiff_codegen.cpp) | 14,545 | All 3 AD modes, vector calculus |
...
**Complex numbers and AD:** a derivative with respect to a real parameter passes through complex values on both engines ([ADR-0025](../design/adr/0025-complex-values-carry-derivatives.md)). A complex number is a pair of real numbers, and under differentiation each component is an ordinary numeric-tower value. On the native engine a complex value whose component carries a derivative stores the plain `{real, imag}` pair first and the two components after it as tagged values (`eshkol_complex_carrier_t`, marked by `ESHKOL_COMPLEX_CARRIER_FLAG`), so a reader that does not know about carriers still reads the value; every complex operation on it (`+ - * /`, `real-part`, `imag-part`, `magnitude`, `angle`, `conjugate`, `make-polar`, `exp`, `log`, `sqrt`, `sin`, `cos`, `tan`) is its component formula evaluated by the generic real arithmetic, so a jet, a tape node and a Taylor tower all flow through with the rules they already have. On the VM, whose forward carrier is a first-order dual, `VmComplex` carries the matching tangent pair and each complex operation propagates it by its own rule (holomorphic functions by `f'(z) dz`). Where no derivative exists — `magnitude` and `angle` at `0+0i`, `log` and `sqrt` at the origin — a derivative raises instead of answering 0. Differentiating with respect to a complex evaluation point (a Wirtinger or complex derivative) is not part of `derivative`, which is defined over the reals; both engines refuse a complex point with a message.
