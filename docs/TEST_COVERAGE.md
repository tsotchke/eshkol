...
suites**; CTest **198/198**; the value-position and compound-accessor regression is green, and this result as of the release
...
and 184/184 figures, which were correct on an earlier commit but had drifted
as the suite grew. The v1.3.5-evolve roll-up is regenerated by the release
battery. See [TESTING.md](TESTING.md) for how each gate is run and
...
| `ctest --test-dir build` | 198/198 tests | The whole CTest suite, including the VM standalone smoke target; gated by `scripts/run_ctest_gate.sh` and read by the release oracle |
