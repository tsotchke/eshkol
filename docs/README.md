...
- [VM Parity](VM_PARITY.md) — bytecode-VM vs native-codegen parity ratchet: 961 rows (604 `vm-supported`, 46 `native-only-justified`, 311 `gap`), differential 338/338
...
- [Bindings reference index](reference/bindings/INDEX.md) — fans out to:
  - [python.md](reference/bindings/python.md) — `import eshkol`, the `Context.eval`/`derivative`/`gradient` API, and the NumPy capsule lifetime guarantee (audit H1, #458)
...
- [ADR 0016 — One boundary for each place a value crosses a representation](design/adr/0016-ad-value-boundaries.md) — a gradient point written as separate scalars is parsed as the vector literal it stands for, and a list cell is read and written whole, so a derivative carrier survives both.
