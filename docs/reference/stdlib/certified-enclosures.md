...
| `ia-atan` | Half-angle reduction `atan(x) = 2·atan(x/(1+sqrt(1+x²)))`, an algebraic identity valid for every real `x` (no sign/domain casework), applied recursively via `ia-sqrt` until the argument's magnitude ≤ 1/2. `ia-atan-core` then uses the Gregory-Leibniz series (coefficients `(-1)^i/(2i+1)`, no factorials at all) with the **alternating-series remainder theorem**: for an alternating series with strictly-decreasing term magnitudes (true for `\|u\| <= 1`), the truncation error is bounded by the first omitted term's magnitude. |
...
than once (e.g. `(ia* a a)` inside `ia-atan`'s reduction, or a Horner
evaluation of a polynomial in `t`) as if the occurrences were independent:
...
loosens with domain radius (see §3); `tests/stdlib/certified_enclosures_test.esk`
