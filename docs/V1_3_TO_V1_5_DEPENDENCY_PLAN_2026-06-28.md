...
- ICC `readiness --repo eshkol_lang --target v1.3-evolve` is backed by a real
  smoke command, not only static evidence. Current status is ready/100 on the primary macOS arm64 host
...
1. Finish the v1.3 core gate.
   - ICC smoke evidence for `v1.3-evolve` is fixed on the primary macOS arm64 host.
   ...
   - ESH-0087 fleet certification is complete: every self-hosted lane (Linux x64,
     Linux arm64, Windows x64, and the Jetson CUDA lane) has focused v1.3
     feature + AD/input2 PASS evidence.
...
- ESH-0086 and ESH-0087 are done locally: ICC smoke evidence is fixed and mesh
  certification has PASS evidence on the Linux, Windows, and Jetson Nix paths.
