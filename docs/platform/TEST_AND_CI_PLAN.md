...
- `scripts/remote_windows_verify.sh`
  ...
  - Jack gate example:
    `scripts/remote_windows_verify.sh <windows-host> --build-dir <build-dir> --suite-only`
...
- `tests/toolchain/atomic_ops_test.cpp`
  - verifies typed atomic load/store/exchange/compare-exchange/fetch-add/fetch-sub/fetch-and/fetch-or/fetch-xor parsing, ordering diagnostics, and LLVM atomic IR lowering
