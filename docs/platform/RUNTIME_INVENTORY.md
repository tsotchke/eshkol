...
| `lib/core/arena_memory.cpp` | `runtime-core` | split required | Core allocator and tagged-object substrate. Hosted process stack-limit setup, FILE-backed string ports, and arena/hash-table sync primitives have been split out. Remaining work is to split current port/display/reader and higher-level data-structure helpers. |
...
4. Continue splitting `arena_memory.cpp` along host-dependent seams. The
   ...
   lifecycle, process stack-limit, FILE-backed string-port, arena sync,
