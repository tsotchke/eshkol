...
| `lib/core/runtime.cpp` | `runtime-core` API + `runtime-hosted` backend | split required | Interface is foundational, but current behavior depends on process state and OS shutdown semantics. The generated-code helper groups, operation-tracking state, shutdown-hook registry, and hosted signal installation have been split out. |
...
4. Continue splitting `runtime.cpp` and `arena_memory.cpp` along
   ...
   into dedicated runtime-core units, and moved the current hosted
   fatal/type-error, dynamic-parameter, in-flight operation, shutdown-hook, and
   signal sinks into dedicated hosted units; the remaining file still needs
   lifecycle boundaries separated.
