...
| `lib/core/runtime_bytevector.cpp` | `runtime-core` | direct | Bytevector allocation, length, ref/set, and copy helpers used by generated bytevector code. Uses tagged object layout and arena allocation; error paths delegate to the remaining runtime fatal helper until panic hooks split. |
...
4. Continue splitting `runtime.cpp` and `arena_memory.cpp` along
   ...
   into dedicated runtime-core units; the remaining file still needs lifecycle,
   fatal-error, and parameter boundaries separated.
