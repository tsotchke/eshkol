...
| `lib/core/arena_memory.cpp` | `runtime-core` | split required | Core allocator and tagged-object substrate. Hosted process stack-limit setup, FILE-backed string ports, arena/hash-table sync primitives, the FILE-backed S-expression reader, hosted display/current-port helpers, and closure reflection/lambda-registry helpers have been split out. Remaining work is to split higher-level data-structure helpers. |
...
4. Continue splitting `arena_memory.cpp` along host-dependent seams. The
   ...
   current hosted fatal/type-error, lifecycle, process stack-limit, FILE-backed
   string-port, reader, display, arena sync, dynamic-parameter, in-flight
   operation, shutdown-hook, and signal sinks are dedicated runtime-hosted
   units.
