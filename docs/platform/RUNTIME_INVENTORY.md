...
| `lib/core/arena_memory.cpp` | `runtime-core` | split required | Core allocator and tagged-object substrate. Hosted process stack-limit setup, FILE-backed string ports, arena/hash-table sync primitives, the FILE-backed S-expression reader, hosted display/current-port helpers, hosted exception/forward-reference diagnostics, closure allocation helpers, closure reflection/lambda-registry helpers, continuation/dynamic-wind helpers, AD tape/node helpers, hash-table helpers, tensor allocation helpers, tensor math helpers, list/error helper ABI, shared/weak-reference helpers, region/thread-local arena helpers, deep structural equality, header-aware object allocation helpers, and tagged cons allocation/accessors have been split out. Remaining work is to split the remaining raw allocator and C++ wrapper groups. |
...
3. Classify the current hosted logger and resource-limit implementations as
   ...
   families.
4. Continue splitting `arena_memory.cpp` along host-dependent seams. The
   `runtime.cpp` split has reduced that file to the runtime-core interrupt flag;
   freestanding-safe tensor index, tensor fill, string, bytevector, deep
   equality, closure
   reflection/lambda-registry, continuation/dynamic-wind, hash-table, tensor
   allocation, and automatic-differentiation helpers are dedicated runtime-core
   units, and the
   current hosted fatal/type-error, exception handling, lifecycle, process
   stack-limit, FILE-backed string-port, reader, display, arena sync,
   dynamic-parameter, in-flight operation, shutdown-hook, and signal sinks are
   dedicated runtime-hosted units.
