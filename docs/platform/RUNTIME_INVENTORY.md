...
| `lib/core/arena_memory.cpp` | `runtime-core` | split required | Core allocator and tagged-object substrate. Must be split to remove pthread mutex setup from the core slice. Hosted process stack-limit setup and FILE-backed string ports have been split out. |
...
4. Continue splitting `arena_memory.cpp` along host-dependent seams. The
   ...
   lifecycle, process stack-limit, FILE-backed string-port, dynamic-parameter,
   in-flight operation, shutdown-hook, and signal sinks are dedicated
   runtime-hosted units.
