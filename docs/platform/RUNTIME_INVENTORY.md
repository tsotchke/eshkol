...
| `lib/core/runtime_arena_cpp.cpp` | `runtime-split-pending` | split required | C++ `Arena` RAII wrapper around the C arena ABI. It is separated from raw allocator mechanics but still needs a final family decision because it exposes exceptions and C++ convenience semantics rather than generated-code runtime ABI. |
...
3. Classify the current hosted logger and resource-limit implementations as
   ...
   families.
4. Continue shrinking the remaining split-pending runtime surface. The
   `arena_memory.cpp` implementation has been decomposed into raw runtime-core
   arena mechanics, hosted diagnostics policy, and a separate C++ wrapper seam;
   the remaining split-pending source is `runtime_arena_cpp.cpp`.
