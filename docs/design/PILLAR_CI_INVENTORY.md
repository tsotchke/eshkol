...
| `check_surface_counts.py` (release-record consistency; `--self-test`) | `ci.yml` `assurance-gates` self-test, ~L645; `pillars-readiness` `--no-trace`, ~L1185 | required, every PR (the `pillars-readiness` step on every non-docs PR) |
| `check_changelog_completeness.py` (every merged PR in `previous_tag..HEAD` is referenced by the changelog release section or listed in `tests/coverage/changelog_no_user_facing_change.json`; build-free) | `ci.yml` `assurance-gates`, self-test ~L646, gate ~L850 (full-history checkout, `--pending-pr` for the PR under review) | required, every PR including docs-only |
| `check_doc_front_matter.py` (documentation front matter, evergreen wording, labelled links to dated pages, ratcheted unmarked-page count) | `ci.yml` `assurance-gates`, self-test ~L647, gate ~L867 | required, every PR including docs-only |
| `doc_audit/check_doc_examples.py` (every fenced example in the `GATED_SCOPES` scopes runs on JIT and AOT and prints what the page says) | `ci.yml` `assurance-gates` self-test, ~L651; `unix-matrix` `linux-x64-lite` and `macos-arm64-lite` step, ~L2181; `run_v1_3_release_producers.sh` | required lanes; a change to a gated page classifies as `tests-only`, so these lanes run for it |
| `check_engine_parity_threshold.py --self-test` (exact integer-count floors) | `ci.yml` `assurance-gates`, ~L653 | required, every PR |
| `run_release_invariant_probes.sh` (ABI layout, object-header ratchet, closed-enum dispatch and AD-exactness receipts before the architecture grade) | `run_icc_smoke.sh` (shared probe recipe) and `release.yml` release-readiness phases | release-time; receipt contracts are CTests (`test_release_invariant_contracts`, `test_release_probe_receipts`) |
...
| `scripts/doc_audit/*.py` (8 files) | Extract/execute every fenced example across 358 markdown docs. Maintenance-pipeline shaped (regenerates doc examples), not a per-PR or nightly correctness gate; zero workflow references before and after this PR. The example gate (`check_doc_examples.py`) and the self-tests of `check_results_schema.py` and `test_harness.py` are wired in CI (see A). |
