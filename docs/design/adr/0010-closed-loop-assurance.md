...
| **A10** — toolchain-driver / type-checker / packaging outside the exposure manifest | ARCHITECTURAL | New manifest categories `toolchain_flag` / `type_diagnostic` / `package_surface` from `cli-flag-audit` + the type-checker error enum + the install manifest; each gets a floor; packaging lane installs and runs the artifact | **P8** (packaging lane) + manifest extension | v1.4 | Planned |
| **A11** — diagnostic-quality is a single boolean | ARCHITECTURAL | Diagnostic golden-corpus (input → expected diagnostic code + span) gated per surface family | manifest extension (`type_diagnostic`) | v1.4 | Planned |
| **A12** — leak/RSS native-only; ASan leak detection off | ARCHITECTURAL | LSan lane with an arena/JIT-cache suppression file (not blanket `detect_leaks=0`); extend `region_evac` POISON to the VM route | CI lane + P8 workload-shaped RSS | v1.4 | Planned |
