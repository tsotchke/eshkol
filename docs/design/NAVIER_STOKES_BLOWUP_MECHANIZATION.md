...
| 7 | Section 3.1 | Derive the core extents `ℓr ≍ τ^{1/2}`, `ℓz ≍ τ^{1/2−h}`, `ℓz/ℓr ≍ τ^{−h}` and the growth `‖u_θ⁽⁰⁾‖_{L∞(Cτ)} ≍ τ^{−1/2−h}` as an exactly solved exponent system | Exact rational linear solve: `(solve A b n)` (`lib/math.esk`, LU with partial pivoting) over rational entries; `det`/`inv` in the same module | SHIPPED | v1.3.0-evolve | `ns_similarity_exponents_solved` |
| 8 | Section 3.5 | Derive `E_core ≍ τ^{3/2−h} τ^{−1−2h} = τ^{1/2−3h}` and `D_core ≍ τ^{−1/2−3h}`, then verify `∫₀^{τ₀} τ^{−1/2−3h} dτ = τ₀^{1/2−3h}/(½ − 3h) < ∞` exactly for `h < 1/6` | Exact rational exponent arithmetic plus a certified inequality on the exponent; interval endpoints via `interval-lo`/`interval-hi` (`lib/core/ad/interval.esk`) | SHIPPED | v1.3.0-evolve | `ns_core_energy_exponents` |
...
| 12 | Section 3.2; Lemma A.8 | Impose the total-moment identities `∫₀^∞ r² R_θ⁽⁰⁾ dr = 0` and `∫₀^∞ r R_z⁽⁰⁾ dr = 0`, which force the stress to vanish beyond the exterior radius as well as near the axis | Cumulative-integral evaluation plus an exact zero test on the resulting rational; a *certified* zero over an unbounded range needs the interval/Taylor-model tail bound of row 22 | SHIPPED (compact range) | v1.3.0-evolve | `ns_cumulative_moment_identity` |
| 13 | (4.15) | Form the five cumulative radial integrals `M = ∫₀^X U dx`, `I = ∫₀^X H dx`, `J = ∫₀^X UH dx`, `S = ∫₀^X (U² − E²/2) dx`, `C_p = ∫₀^X E²/(2x) dx`, with `Π = Π(0,η) + C_p` | `integrate`; near the axis the same five as exact Taylor coefficients of the integrands under `taylor` (exact-coefficient mode) | SHIPPED | v1.3.0-evolve | `ns_cumulative_moment_identity` |
...
| 17 | (4.21) | Evaluate `U(P_c, J_c) = P_c + J_c²/4 − \|J_c\|·√((P_c − 2)/2 + J_c²/16)` and test the relaxed cone condition `P_c > 2`, `v_s < U(P_c, J_c)` | Exact rational arithmetic with a certified square root: `taylor-model` enclosure of `√·` (`lib/core/ad/taylor_models.esk`), compared through `interval-contains?` | SHIPPED | v1.3.0-evolve | `ns_cone_condition_equivalence` |
| 18 | Lemma 4.5; (4.22) | Certify the equivalence: for `v_s > 2`, the admissible cone condition holds iff `P_c > v_s` and `(v_s − 2)J_c² < 2(P_c − v_s)²` — a root computation for the quadratic `2(P_c − v)² − (v − 2)J_c²` in `v` | Exact quadratic-root algebra over rationals; `taylor-root` (`lib/core/ad/taylor_numerics.esk`) as the numeric cross-check | SHIPPED | v1.3.0-evolve | `ns_cone_condition_equivalence` |
| 19 | Lemma 4.5, second assertion | Produce the uniform threshold `P_K = max{(B_K + 2)/c_K, 8B_K/γ_K}` on a compact parameter set `K`, from the compactness bounds `c ≥ c_K`, `G ≥ γ_K`, `v ≤ B_K`, `cv ≤ B_K` | BUILD ITEM: **certified compact-set bounds** — interval-arithmetic supremum/infimum over a parameter box, driven by `taylor-model` enclosures with subdivision | PLANNED | v1.5.0-intelligence | `ns_cone_condition_equivalence` |
...
| # | Paper reference | Operation | Eshkol primitive or BUILD ITEM | Status | Version | Gate |
|---|---|---|---|---|---|---|
| 22 | Lemma A.1 | Certify that the moment matrix `B_ij = ∫₀^∞ x^{α_i} β_j(x) dx` is invertible for distinct real exponents `α_i` and ordered bump supports `I₁ < ⋯ < I_m` — a sign argument on `det[x_j^{α_i}]` plus multilinearity | Exact rational determinant `(det M n)` (`lib/math.esk`) on the sampled moment matrix; the generic sign argument is the BUILD ITEM of row 3 | SHIPPED (instances) / PLANNED (generic) | v1.3.0-evolve / v1.4.0-connection | `ns_moment_matrix_invertible` |
| 23 | (A.1) | Quantify the loss when exponents collide: for translated bumps, `det B = b(s₁)b(s₂)(e^{s₂Δ} − e^{s₁Δ})`, giving `‖B^{−1}‖ = O(λ^{−1})` at separation `λ` | Exact rational algebra plus a certified `exp` enclosure via `taylor-model` | SHIPPED | v1.3.0-evolve | `ns_moment_matrix_invertible` |
| 24 | Lemma A.2; (A.2)-(A.3) | Solve the quadratic moment system `F_η(c) = B(η)c + Q_η(c,c) = d(η)` by the contraction `c ↦ B^{−1}(d − Q(c,c))`, under `8β₀²κ₀d₀ ≤ 1`, and bound `‖c‖_{C^k} ≤ 2β_k‖d‖_{C^k}` | `(solve A b n)` for the linear part; the fixed-point iteration in plain Eshkol; the contraction hypothesis checked as a certified inequality on `β₀`, `κ₀`, `d₀` | SHIPPED | v1.3.0-evolve | `ns_moment_quadratic_solve` |
...
| 27 | Lemma A.6; (A.33)-(A.38) | Build the heat exterior: `K(r,t) = c_∞ s^{−A} H(2τ/s)`, `s = r²/2`; verify `Z²H'' + (1 + 2a_K Z)H' + a_K(a_K − 1)H = 0`, the derivative values `H^{(m)}(0) = (−1)^m (h)_m (1 + h)_m`, and the expansion `H(Z) = 1 − h(1 + h)Z + O_h(Z²)` | `taylor` in exact-coefficient mode: the rising-factorial coefficients `(h)_m(1+h)_m` are exact rationals in `h`, and substituting the truncated series into the ODE must annihilate every coefficient | SHIPPED | v1.3.0-evolve | `ns_heat_exterior_recurrence` |
| 28 | Lemma A.6, monotonicity | Certify `K_r < 0` and `0 ≤ −ZH'/H < h`, i.e. `rK_r/(2K) = −A − ZH'/H < −½` | Certified enclosure of the ratio `ZH'/H` over the domain via `taylor-model` and `interval-contains?` | SHIPPED | v1.3.0-evolve | `ns_heat_exterior_recurrence` |
...
| 30 | Lemma A.8; Proposition A.10 | Under the exact moment conditions `M(∞) = J(∞) = S(∞) = 0`, `∫₀^∞ (H − H_pow) dX = 0` and the normalized `Π`, conclude the leading tangential stresses vanish for `X ≥ X_b`, with the backward stress formula (A.46) valid (no boundary term, by the tail bounds (A.44)) | Exact zero tests on the four moments; the convergence of the subtracted angular moment needs the certified tail bound BUILD ITEM of row 19 | SHIPPED (moments) / PLANNED (tail) | v1.3.0-evolve / v1.5.0-intelligence | `ns_cumulative_moment_identity` |
...
| 36 | Lemma C.1, removable singularity | Certify that `G(p)/p = ∫₀¹ G'(up) du` removes the apparent singularity at `p_{s,2} = 0`, giving joint smoothness and the value `t = t_s + d₀ µ sin θ'` | Exact series division: `taylor` in exact-coefficient mode divides out the common factor with zero floating error | SHIPPED | v1.3.0-evolve | `ns_shear_loop_mean` |
...
| 53 | (6.14) | Bound the covering-index spread `\|i(ℓ) − i(ℓ')\| ≤ 1 + (4(1+h)log 2 + 8/ℓ₀)/log T_g ≤ Δ_max` | Certified logarithm enclosure via `taylor-model`, then an exact integer ceiling | SHIPPED | v1.3.0-evolve | `ns_torus_support_disjointness` |
...
| # | Paper reference | Operation | Eshkol primitive or BUILD ITEM | Status | Version | Gate |
|---|---|---|---|---|---|---|
| 55 | Lemma 7.1; (7.2)-(7.3) | Construct the phase and a frame `B` orthogonal to its gradient, on neighborhoods of diameter `O(S_*^{−3})` | `cross`, `dot`, `normalize` (`lib/math.esk`) for the frame; the phase is a closed expression differentiated by `gradient` | SHIPPED | v1.3.0-evolve | `ns_pulse_amplitude_envelope` |
...
| 59 | Proposition 7.5; (7.24), (7.29) | Solve the 2x2 system for the two pulse-family squared amplitudes: `y = H^{−1}T_{0,*}`, `a_σ = √(y_σ)`, i.e. `h_± y_± = ½(−T_N/A_c ∓ T_K/u_*)`; certify `y_σ > 0`, `\|det H\| ≥ c/S_*`, `‖H^{−1}‖ ≤ C√S_*`, and hence `C(W₀) = ε T_{0,*}` | `(solve A b n)` at `n = 2` over exact rationals, plus `det`; positivity is an exact sign test. This is the arithmetic core of the momentum-transport step | SHIPPED | v1.3.0-evolve | `ns_covariance_two_family_solve` |
...
| 70 | Lemma 8.8; (8.27); Section 8.7 | Record the exact difference between the linearized correction and the updated defects, certify the improved decay of the nonlinear remainder, and check chart-to-chart compatibility of the recomputation | Exact-difference arithmetic; the decay improvement is a certified exponent inequality | SHIPPED (algebra) / PLANNED (composition) | v1.3.0-evolve / v1.6.0-reasoning | `ns_residual_exponent_ladder` |
...
| 72 | Proposition 9.1; Lemma 9.2 | Bound the residual of velocities constructed by curls, and the wave-mean interaction terms, per harmonic class | Certified bounds through `taylor-model` enclosures over the chart box | PLANNED | v1.5.0-intelligence | `ns_residual_exponent_ladder` |
...
| 80 | Lemma 10.2; Lemma 10.3; (10.5), (10.8)-(10.9) | Certify that `∂ˣ_α ∂ᵗ_j f` converges uniformly on `R³` as `t ↑ 1`, using the flatness bound near the origin, the identically-zero residual for `X ≥ X_ext`, and the heat bounds plus the normalized pressure integral (3.5) in the remaining region; then realize the limits from `t > 1` with `f ∈ Cc∞(R³ × (0,∞); R³)` supported in `K × [0,2]` | Certified enclosures per derivative order (row 78) plus the heat-exterior bounds of rows 27-28 | PLANNED | v1.6.0-reasoning | `ns_localization_smooth_extension` |
| 81 | Lemma 10.4; (10.13) | Certify the global energy `F(1) < ∞` and the integrated dissipation bound directly from the equation for the localized fields | Cumulative-integral inequality on a compact interval with certified arithmetic: `integrate` for the value, `taylor-model` for the tail, `interval-contains?` for the verdict | SHIPPED (compact range) / PLANNED (tail) | v1.3.0-evolve / v1.5.0-intelligence | `ns_energy_dissipation_bounds` |
| 82 | Lemma 10.5; (10.20)-(10.21) | Along `x_τ = (√(2X_in τ), 0, 0)`, `t = 1 − τ`, verify `u_θ(x_τ, 1 − τ) = τ^{−A}(e₀ + O(τ^{2h})) → +∞`, exclude a classical `H³` continuation through time one via `H³(R³) ↪ L∞(R³)`, and run the difference-energy plus Gronwall comparison | The growth path is evaluated directly; the comparison argument is the certified-inequality pattern of row 81 | SHIPPED (growth path) / PLANNED (comparison certificate) | v1.3.0-evolve / v1.6.0-reasoning | `ns_energy_dissipation_bounds` |
...
| Exact-coefficient Taylor towers | `taylor`, `derivative-n` (compiler builtins) | `lib/frontend/parser.cpp` (parse of `(taylor f x k)` / `(derivative-n f x k)`), `lib/backend/autodiff_codegen.cpp` (`AutodiffCodegen::taylorApiCore`), `lib/core/runtime_taylor.c` (`alloc_exact_series`, `taylor_pow_exact`) | v1.3.0-evolve | `ad_taylor_p6_exact_coefficients` |
...
| Interval arithmetic and Taylor models | `taylor-model`, `tm-range`, `tm-eval`, `interval-lo`, `interval-hi`, `interval-contains?` | `lib/core/ad/taylor_models.esk`, `lib/core/ad/interval.esk` | v1.3.0-evolve | `ad_taylor_p8_taylor_models` |
...
| Numeric linear algebra and quadrature | `(det M n)`, `(inv M n)`, `(solve A b n)` (LU with partial pivoting), `(integrate f a b n)` (Simpson), `(newton f df x0 tol iters)`, `cross`, `dot`, `normalize` | `lib/math.esk`; `docs/reference/stdlib/math.md` | v1.1-accelerate | `stdlib-ready` |
...
**Engine qualifier.** Of the surface above, `taylor` and `derivative-n` are
compiler builtins; the `core.ad.*` modules are Eshkol library code bottoming out
in those builtins, and `lib/math.esk` is ordinary Eshkol. The bytecode VM does
not carry the Taylor-tower surface — that is the existing BUILD ITEM recorded in
[ROADMAP.md](../../ROADMAP.md) (VM Taylor-tower builtins, target v1.4.1), and it
applies to this document too: everything in Section 2 that names `taylor` or
`derivative-n` runs today on the LLVM backend, AOT and JIT.
...
| `mathematics_navier_stokes_similarity_scales.esk` | 1, 2, 7, 8 | The similarity scale exponents `(A, D, ℓr, ℓz, E_core, D_core)` derived as an exactly solved linear system over rationals, with the finite-energy condition `h < 1/6` verified exactly | IN PROGRESS | v1.3.5 | `ns_similarity_exponents_solved` |
...
| **Certified compact-set bounds.** Interval-arithmetic supremum/infimum over a parameter box with adaptive subdivision, driven by the shipped `taylor-model` enclosures, so a compactness constant is produced rather than asserted | Steps 19, 30 (tail), 72, 77, 81 (tail) | v1.5.0-intelligence | `ns_cone_condition_equivalence` |
...
2. **Every appendix lemma is certified.** Each numbered statement in
   ...
   a certified inequality with explicit interval endpoints and rounding
   direction. Steps 22-39. A lemma whose Eshkol form is a floating-point
   comparison is not certified and must be recorded as an open BUILD ITEM, never
   as a pass.
...
shipped plus the two v1.4.0-connection items. Condition 2 needs the torus and
compact-set items at v1.5.0-intelligence. Condition 3 is the assurance-track
deliverable and is what makes the result citable by someone who does not run
this compiler.
...
construction that is not yet mechanized.
