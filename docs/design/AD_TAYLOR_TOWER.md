...
- **Tag-gated combination.** Binary ops (`mul`, `div`, …) only *combine perturbations* of towers whose epoch tag equals the current context's tag. Two series are combined only when their tags are **equal**, which is what makes confusion impossible; what happens to a tower carrying a **foreign** tag is §5c.
...
### 5c. A foreign level is a COEFFICIENT, not a constant (ESH-0413)

§5a says a foreign-tagged tower is not differentiated at this level. It does **not** say the foreign tower must be *flattened to its `c[0]`*, and flattening is where every nesting limit came from. `c[0]` is the enclosing level's value with its own series thrown away, so the enclosing pass reads "no dependence" and answers **zero**. ESH-0412 bought one level back by routing the foreign `c[1]` onto a single first-order companion series — enough for one enclosing level at first order, and a ceiling everywhere else:

```scheme
;; both passes above first order: raised
(taylor (lambda (t) (* t (derivative-n (lambda (r) (* r r r r)) (+ 1 t) 2))) 0 2)

;; two enclosing levels over an order-2 inner pass: answered 0, silently
(define (g t1 t2) (derivative-n (lambda (r) (* r r r r)) (+ 1 t1 t2) 2))
(derivative-n (lambda (t1) (derivative-n (lambda (t2) (g t1 t2)) 0 1)) 0 1)   ; => 0, want 24
```

The root form is the one this document has named since §1: **a carrier is a jet over a jet.** Each differentiation pass owns a **level** (its epoch), and a carrier at level *L* of order *K* is the truncated series in *L*'s perturbation whose **coefficients are themselves carriers of the enclosing levels** — a truncated multivariate Taylor tower indexed by `(level, order)`. Concretely (`ESH_TAYLOR_COEFF_CARRIER`, `inc/eshkol/eshkol.h`):

- **Coefficients are tagged values.** A CARRIER tower reinterprets its `c[]` storage as `eshkol_tagged_value_t[K+1]`, exactly as the EXACT tier does, but the entries may be *any* tagged number — including another tower whose epoch names a strictly enclosing level. Unlike the EXACT tier there is no homogeneity rule: `c[0]` may be an enclosing carrier while `c[1]` is an exact `1`.
- **Foreign level ⇒ coefficient-valued constant.** `normalise_operand_carrier` puts the **whole foreign tower**, structure and level tag intact, into `c[0]` of the active level's operand series. It is a constant *of this level* — its order-≥1 coefficients still belong to the level that owns them — and it is not projected to a number, so nothing is lost and nothing has to be refused.
- **The recurrences are polymorphic in the coefficient type.** The `trc_*` kernels in `runtime_taylor.c` are the `tr_*` algebra written over "whatever the coefficients are": every scalar operation goes through `coeff_add/sub/mul/div/unary/pow`, which dispatch back into the tower kernel when an operand is itself a carrier. The kernel that differentiates doubles is therefore literally the kernel that differentiates towers, and **depth is bounded only by the levels a program opens.**
- **Seeding and extraction.** `eshkol_taylor_seed_tagged` opens a level *over* whatever the point already is: `c[0]` = the enclosing carrier, `c[1]` = 1. `eshkol_taylor_extract_tagged` returns `k!·c[k]`, which is again a carrier of the enclosing level — precisely what the enclosing pass reads — and `eshkol_taylor_coeffs_list` does the same coefficient by coefficient for `taylor`.
- **The 8-jet boundary.** `derivative`/`gradient`/`hessian` carry three simultaneous first-order perturbations in one flat jet. A pass that would seed a jet but meets a tower level runs as an **order-1 level** instead (`eshkol_ad_nested_seed`). In the other direction, a *single* enclosing jet direction is exactly what the §8 first-order companion holds, and it keeps holding it — that is the cheap path, and it is the one a forward-over-forward Hessian-vector product takes in a loop. **Two** enclosing directions is where a companion runs out: the outermost tower pass then registers a pseudo-epoch per direction (allocated from the same monotonic counter, *before* its own, so "enclosing level < inner level" holds), every occurrence of a direction inside that pass maps to the same level, and the pass's extraction restates the answer as a jet again. While a **reverse** pass is live the jet's slots belong to the ESH-0093 mixed-mode protocol, and the level machinery stands down for the duration.
- **Exactness survives.** The coefficient algebra is the exact numeric tower wherever the coefficients are exact, so a nested pass at an exact point still comes back exact — the ESH-0412 narrowing (a companion series of raw doubles) is gone with the companion.

Consequences: `esh_taylor_t.carry_epoch`, `eshkol_ad_tower_carry_result` and `eshkol_ad_nested_capture_unsupported` are retired — with levels there is no companion to carry and no shape to refuse. Cost at the deepest shape measured (depth 3, order 4 at every level: 5³ = 125 coefficients) is ≈0.26 ms per nested differentiation.

**Gate:** a dedicated nested-AD confusion suite — `(derivative (λ(x) (* x ((derivative (λ(y) (* x y))) 2))))` and friends — whose analytic answers are known, plus the existing P6a nested cells to d=8. A confusion bug shows up as an off-by-a-term derivative and is caught by the analytic reference. Since ESH-0412 the gate is the full 3×3 operator matrix at inexact **and** exact seeds, with the outer variable **captured** by the inner differentiand: `tests/ad/nested_operator_matrix_test.esk` (JIT + AOT), alongside the point-nested matrix in `tests/ad/ad_carrier_nesting_test.esk`. Since ESH-0413 it also carries the **order and depth sweep** over that matrix — `tests/ad/nested_towers_matrix_test.esk`, 155 checks: the depth-2 operator × order matrix with orders 1..3 on each side, the depth-3 operator cube, the full 1..3 order sweep with three live levels, nesting through the evaluation point, exact seeds, and the confusion controls. Its negative controls are the Siskind–Pearlmutter cases in the other direction — an inner pass that does *not* depend on the outer variable must stay a constant — so the two failure modes are pinned against each other rather than one being traded for the other.
...
- **JET4 unchanged; JET8 subsumed in P3 (ESH-0188, done):** P3 found that full removal of the 8-jet is **not yet achievable** without pulling in work from later phases. `gradient` over a 2-level-nested `derivative` chain (ESH-0117) is a genuine *simultaneous* 3-directional mixed partial (inner forward variable, outer forward variable, reverse-seed direction) — mathematically the 2³ hyper-dual basis, not a univariate series. The P1/P2 univariate tower has no way to hold three orthogonal directions at once; expressing that exactly requires either GUW multivariate directional propagation (§7, P4/ESH-0189) or tower-valued reverse-tape primals/adjoints (§8, P5/ESH-0190) — both later, independently-gated phases. Building either early inside P3 would be undocumented scope creep across phase boundaries.
  P3 therefore: (1) deleted the dead compile-time-depth predecessor of the perturbation-tagging mechanism (`seedDerivativeInput`/`extractDerivativeResult`, superseded by ESH-0070's runtime-level mechanism, zero remaining call sites), (2) replaced the magic field-index seeding (`{1}`/`{2}`/`{4}`) with named `DualJetSlot` constants (`kJetE1`/`kJetE2`/`kJetEp`) so the three-perturbation discipline reads consistently with the tower's `EPOCH_TAG` model, and (3) documented the 8-jet in `lib/backend/autodiff_codegen.cpp` as a **bounded, thin adapter**: it never grows past exactly 3 simultaneous directions (unlike an unbounded nested jet, which is the actual wall the tower design eliminates), it only activates when `detectPureDerivChain` falls through (depth < 3) AND a reverse tape is live, and JET4 (order ≤ 2, no live reverse tape) is untouched — the `ep` fields stay compile-time-constant zero and fold away. PR #138's ESH-0117 tests pass unchanged (same code path, not rerouted) — see `tests/ad/reverse_nested_forward_test.esk`. Full removal is re-opened once P4 and/or P5 land.
...
### P9. Differentiable control flow (ESH-0194, done)

**Verdict (2026-07, ESH-0194): the substrate needed almost none of the codegen surface sketched below.** A Taylor tower is a tagged `HEAP_SUBTYPE_TAYLOR` heap value; ordinary control flow (SSA phis, loop-carried variables, recursive-call arguments, closure captures, `map`/`fold` combinators) already threads any tagged heap value through unchanged, because none of that machinery special-cases numeric representation. Empirically verifying every construct in the bullets below — `if`/`cond`/`case` branch selection, named-let loops, recursion (including mutual tail recursion), and `map`/`fold` over closures — found exactly **one** real gap: R7RS numeric comparison (`< > = <= >=`) did not recognize `HEAP_SUBTYPE_TAYLOR` as numeric, so `(if (> x 0) ...)` on a tower-valued `x` raised a type error instead of branching on the tower's primal coefficient `c[0]`. `extractAsDouble` already had the tower-to-primal extraction case (ESH-0186); `compare()` just never reached it. The fix is additive, isolated to `lib/backend/arithmetic_codegen.cpp::compare()` (the `numericHeap` heap-subtype predicate and the double-comparison-path dispatch), mirroring the existing bignum/rational/dual-number cases. **No changes were made** to `control_flow_codegen.cpp`, `tail_call_codegen.cpp`, `call_apply_codegen.cpp`, `function_codegen.cpp`, `map_codegen.cpp`, `runtime_taylor.c`, or `autodiff_codegen.cpp` — once `compare()` accepted towers, every construct below verified correct to order 3+ against closed-form analytic references (`tests/ad/diff_control_flow_test.esk`, 38/38 under both `-r` and AOT).

**Kink policy actually adopted:** the branch-selection kink policy implemented (and the only one implemented) is the standard AD subderivative convention — differentiate the arm the primal predicate actually selects, matching JAX/PyTorch autograd/Stan/Julia ForwardDiff. This is a **narrower** policy than the "explicit differentiability error unless both arms agree through K" sketch below; that stricter opt-in policy remains undone and is left as documented future work (it would require evaluating and comparing both arms' derivative series at every branch, which is a nontrivial dual-branch-evaluation feature, not a control-flow-substrate fix). Gate tests exercise branch derivatives strictly away from any kink, per the ESH-0194 spec.

**Known pre-existing bug found (documented, not fixed here):** a closure literal allocated *fresh, inline*, as an argument expression in a named-let loop's tail-call position, when passed directly to `derivative` inside that same loop body, silently corrupts the loop's accumulator (observed ~1e10 garbage instead of the correct sum, with a compiler "Type warning: argument N of 'loop': expected Float64, got Number" at that call site). This reproduces with **no differentiation at all** — a plain undifferentiated call into the enclosing function is already wrong — and does **not** reproduce with an equivalent `do` loop or plain recursion, so it is a named-let TCO loop-variable lowering bug, not an AD- or control-flow-substrate bug. Workaround (matches the P8/#173 precedent of avoiding the named-let/closure mutable-capture bug class "by construction"): hoist the differentiated closure to a `let`-bound variable before the named-let loop, so the loop body only ever calls `derivative`/`derivative-n` on an existing closure reference. See the `closure-hoist-workaround` probe in `tests/ad/diff_control_flow_test.esk`. Fixing the named-let TCO lowering itself is out of scope for this phase (a "minimal additive changes" mandate, and not a P9 control-flow-AD gap) and is left as a follow-up ticket.

**Motivation (original sketch, retained for context):** Real Eshkol programs do not look like straight-line scalar kernels. They use `if`/`cond`/`case`, named-let loops, recursion, closures, higher-order calls, `map`, and `fold`. AD must be robust through those constructs, including nested derivative operators, or the tower remains only a numeric-kernel differentiator.
...
| **P3** | Subsume `AD_JET8` where possible: dead predecessor code removed, levels re-expressed as explicit named tags, 8-jet reduced to a documented bounded thin adapter (full removal blocked on P4/P5 — see §12) | #138's tests still PASS (unchanged route); JET4 retained; confusion-tag terminology unified | **ESH-0188** (done) |
