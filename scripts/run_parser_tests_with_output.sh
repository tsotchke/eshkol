...
echo "========================================="
echo "  Parser Test Suite - Verbose Mode"
echo "========================================="
echo ""
    ...
    if [ $EXPECTS_ERROR -eq 1 ]; then
        ...
        if ./build/eshkol-run "$test_file" -L./build >> "$output_file" 2>&1; then
            ...
            if ./a.out >> "$output_file" 2>&1; then
                ...
                echo -e "${GREEN}  ✅ Runtime failed as expected (exit $EXIT_CODE)${NC}"
                ...
                tail -20 "$output_file" | grep -v "^========" | grep -v "^TEST TYPE" | grep -v "^RUNTIME"
            fi
        else
            echo "----------------------------------------" >> "$output_file"
        ...
        echo "TEST TYPE: Normal test (expected to pass)" >> "$output_file"
        echo "" >> "$output_file"
        ...
        if ./build/eshkol-run "$test_file" -L./build >> "$output_file" 2>&1; then
            ...
            if ./a.out >> "$output_file" 2>&1; then
                ...
                if grep -q "FAIL:" "$output_file"; then
                    echo "FOUND TEST FAILURES IN OUTPUT" >> "$output_file"
                    echo "FINAL STATUS: TESTS FAILED" >> "$output_file"

                    echo -e "${RED}  ❌ Tests within file failed${NC}"
                    echo -e "${RED}  ❌ OVERALL: FAIL${NC}"
                    ((FAIL++))
                    ...
                    # Show output summary
                    echo ""
                    echo "Runtime output (summary):"
                    tail -10 "$output_file" | grep -v "^========" | grep -v "^RUNTIME" | grep -v "^COMPILATION"
                fi
            else
                EXIT_CODE=$?
                echo "----------------------------------------" >> "$output_file"
...
# Run all tests in parser directory
if [ -d "$TEST_DIR" ]; then
    ...
    echo -e "${RED}Parser test directory not found: $TEST_DIR${NC}"
    exit 1
fi

# Create summary
echo "=========================================" | tee -a "$RESULTS_FILE"
echo "  Parser Test Results Summary" | tee -a "$RESULTS_FILE"
echo "=========================================" | tee -a "$RESULTS_FILE"
TOTAL=$((PASS + FAIL))
