...
eshkol_test_isolation_init "ml-out"
...
echo "========================================="
echo "  ML Test Suite - Verbose Mode"
echo "========================================="
echo ""
...
# Run all tests in ml directory
if [ -d "$ML_DIR" ]; then
    echo "Running tests in $ML_DIR..."
    echo ""
    for test_file in "$ML_DIR"/*.esk; do
        if [ -f "$test_file" ]; then
            run_test_verbose "$test_file"
        fi
    done
else
    echo -e "${RED}ML test directory not found: $ML_DIR${NC}"
    exit 1
fi

# Create summary
echo "=========================================" | tee -a "$RESULTS_FILE"
echo "  ML Test Results Summary" | tee -a "$RESULTS_FILE"
echo "=========================================" | tee -a "$RESULTS_FILE"
TOTAL=$((PASS + FAIL))
