...
def load_public_surface_manifest() -> list[dict[str, str]]:
    """Load the reviewed DD-11 header-symbol scope."""
    if not PUBLIC_SURFACE_MANIFEST.exists():
        return []
    entries: list[dict[str, str]] = []
    for raw in PUBLIC_SURFACE_MANIFEST.read_text(encoding="utf-8").splitlines():
        if not raw.strip() or raw.startswith("#"):
            continue
        fields = raw.split("\t", 4)
        if len(fields) < 4:
            raise ValueError(f"malformed public surface row: {raw!r}")
        path, line, kind, symbol = fields[:4]
        description = fields[4] if len(fields) == 5 else "Public API declaration."
        entries.append({"path": path, "line": line, "kind": kind, "symbol": symbol,
                        "description": description})
    return entries


def public_surface_anchor(symbol: str) -> str:
    return symbol.lower().replace(" ", "-")


def render_public_surface(entries: list[dict[str, str]]) -> str:
    out = [
        "# DD-11 public header surface\n",
        "This page is generated by `scripts/gen_api_docs.py` from "
        "`docs/api/public_surface.tsv`. It is the reviewed consumer-facing "
        "subset of the larger generated header index.\n",
        f"**Coverage scope:** {len(entries)} header symbols.\n",
        "Each entry has a stable anchor so links in `INDEX.md` and the DD-11 "
        "coverage gate cannot silently point at a page without the symbol.\n",
    ]
    for entry in sorted(entries, key=lambda e: (e["symbol"].lower(), e["path"], e["line"])):
        anchor = public_surface_anchor(entry["symbol"])
        out.append(f'<a id="{anchor}"></a>\n')
        out.append(f"### `{entry['symbol']}`\n")
        out.append(f"*{entry['kind']}* — `{entry['path']}:{entry['line']}`\n")
...
def render_readme(per_file: list[tuple[Path, list[Symbol]]]) -> str:
    ...
    out.append(
        "The reviewed DD-11 consumer-facing subset is tracked in "
        "[public_surface.md](public_surface.md); the coverage gate verifies "
        "every entry has a source, page anchor, and index link.\n"
    )
...
def render_index(per_file: list[tuple[Path, list[Symbol]]], public_entries: list[dict[str, str]]) -> str:
    rows = []
    for header, symbols in per_file:
        link = out_md_path(header).relative_to(OUT_ROOT).as_posix()
        for sym in symbols:
    ...
    for entry in public_entries:
        rows.append((entry["symbol"], entry["kind"], entry["path"],
                     "public_surface.md#" + public_surface_anchor(entry["symbol"]), True))
...
def main() -> int:
    ...
    global INC_ROOT, OUT_ROOT, PUBLIC_SURFACE_MANIFEST
    INC_ROOT = inc_root
    OUT_ROOT = out_root
    PUBLIC_SURFACE_MANIFEST = args.repo_root / "docs" / "api" / "public_surface.tsv"
    ...
    planned[out_root / "public_surface.md"] = render_public_surface(public_entries)
    planned[out_root / "INDEX.md"] = render_index(per_file, public_entries)
