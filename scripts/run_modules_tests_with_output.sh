...
eshkol_test_isolation_init "modules-out"
...
echo "========================================="
echo "  Modules Test Suite - Verbose Mode"
echo "========================================="
echo ""
...
run_test_verbose() {
    ...
    if grep -q ";;; Expected: Error" "$test_file"; then
        ...
        echo "----------------------------------------" >> "$output_file"
        ./$BUILD_DIR/eshkol-run -L./$BUILD_DIR "$test_file" -o "$ESHKOL_TEST_BIN" >> "$output_file" 2>&1
        COMPILE_EXIT=$?
        ...
        if [ $COMPILE_EXIT -eq 0 ]; then
            ...
            echo "----------------------------------------" >> "$output_file"
            "$ESHKOL_TEST_BIN" >> "$output_file" 2>&1
            RUNTIME_EXIT=$?
...
if [ -d "$TEST_DIR" ]; then
    ...
    echo -e "${RED}Modules test directory not found: $TEST_DIR${NC}"
    exit 1
fi

# Create summary
echo "=========================================" | tee -a "$RESULTS_FILE"
echo "  Modules Test Results Summary" | tee -a "$RESULTS_FILE"
echo "=========================================" | tee -a "$RESULTS_FILE"
TOTAL=$((PASS + FAIL))
