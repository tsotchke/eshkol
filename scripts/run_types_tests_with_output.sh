...
eshkol_test_isolation_init "types-out"
...
echo "========================================="
echo "  HoTT Type System Test Suite - Verbose Mode"
echo "========================================="
echo ""
...
# Run all tests in types directory
if [ -d "$TYPES_DIR" ]; then
    echo "Running tests in $TYPES_DIR..."
    echo ""
    for test_file in "$TYPES_DIR"/*.esk; do
        if [ -f "$test_file" ]; then
            run_test_verbose "$test_file"
        fi
    done
else
    echo -e "${RED}Types test directory not found: $TYPES_DIR${NC}"
    exit 1
fi

# Create summary
echo "=========================================" | tee -a "$RESULTS_FILE"
echo "  HoTT Type System Test Results Summary" | tee -a "$RESULTS_FILE"
echo "=========================================" | tee -a "$RESULTS_FILE"
TOTAL=$((PASS + FAIL))
