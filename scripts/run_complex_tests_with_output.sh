...
eshkol_test_isolation_init "complex-out"
...
echo "========================================="
echo "  Complex & FFT Test Suite - Verbose Mode"
echo "========================================="
echo ""
...
# Run all tests in complex directory
if [ -d "$COMPLEX_DIR" ]; then
    echo "Running tests in $COMPLEX_DIR..."
    echo ""
    for test_file in "$COMPLEX_DIR"/*.esk; do
        if [ -f "$test_file" ]; then
            run_test_verbose "$test_file"
        fi
    done
else
    echo -e "${RED}Error: $COMPLEX_DIR directory not found${NC}"
    exit 1
fi

# Create summary
echo "=========================================" | tee -a "$RESULTS_FILE"
echo "  Complex & FFT Test Results Summary" | tee -a "$RESULTS_FILE"
echo "=========================================" | tee -a "$RESULTS_FILE"
TOTAL=$((PASS + FAIL))
