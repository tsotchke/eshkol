...
eshkol_test_isolation_init "memory-out"
...
echo "========================================="
echo "  Memory Test Suite - Verbose Mode"
echo "========================================="
echo ""
...
run_test_verbose() {
    ...
    if grep -q ";;; Expected: Error" "$test_file"; then
        ...
        if ./$BUILD_DIR/eshkol-run -L./$BUILD_DIR "$test_file" -o "$ESHKOL_TEST_BIN" >> "$output_file" 2>&1; then
            ...
            echo -e "${RED}  ❌ Compilation succeeded but should have failed${NC}"
...
# Run all tests in memory directory
if [ -d "$TEST_DIR" ]; then
    ...
    echo -e "${RED}Memory test directory not found: $TEST_DIR${NC}"
    exit 1
fi

# Create summary
echo "=========================================" | tee -a "$RESULTS_FILE"
echo "  Memory Test Results Summary" | tee -a "$RESULTS_FILE"
echo "=========================================" | tee -a "$RESULTS_FILE"
TOTAL=$((PASS + FAIL))
