...
# Eshkol XLA Test Suite
# Tests XLA backend dispatch and operations on massive tensors
...
# Check XLA availability (optional info)
echo -e "${CYAN}Note: XLA tests will use BLAS/SIMD fallback if XLA is not available.${NC}"
echo -e "${CYAN}      This is expected behavior on lite builds.${NC}"
echo ""

echo "Testing all files in tests/xla/ directory..."
echo ""

# Check if test directory exists
if [ ! -d "tests/xla" ]; then
    echo -e "${YELLOW}Warning: tests/xla/ directory not found. Creating...${NC}"
    mkdir -p tests/xla
fi

# Count test files
TEST_COUNT=$(find tests/xla -name "*.esk" 2>/dev/null | wc -l | tr -d ' ')
...
# Run each test
for test_file in tests/xla/*.esk; do
    ...
    if ./build/eshkol-run "$test_file" -L./build > /dev/null 2>&1; then
        ...
        if ./a.out > /tmp/xla_test_output.txt 2>&1; then
            ...
            if grep -q "error:" /tmp/xla_test_output.txt; then
...
# Clean up
rm -f /tmp/xla_test_output.txt a.out
