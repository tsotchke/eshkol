...
def main() -> int:
    ...
    if len(criteria) != 34:
        raise SystemExit(f"expected 34 authored criteria; found {len(criteria)}")
    ...
    if errors:
        ...
        return 1
    print(f"release evidence verification: PASS ({checked_runtime} runtime events, {checked_tests} named test actions, 34 criteria)")
    return 0
