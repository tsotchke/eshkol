...
- **Every declared executable surface has evidence, and the evidence is execution-backed.** The language-surface manifest is 1,078/1,078. Every one of the 1,078 declared constructs earned its row by running: it dispatched or executed in a passing run. Lexical name-presence no longer counts — it is a diagnostic now, and earns no release credit. The surface was honestly re-baselined upward from 1,056, as the `i128` tower, `linear-solve`, and the string/pointer conversions landed as new core builtins. The coverage floor and deficit ledger are ratcheted to 1,078, so nobody can walk the number back quietly.
- **VM results are explicit, not overstated.** The parity manifest is 936 rows: 562 `vm-supported`, 45 `native-only-justified`, and 329 `gap`. Of those gaps, 17 are verified behavioral divergences with reproducible programs. `op:GRADIENT` and `op:DERIVATIVE` move to `vm-supported` this release, so `gradient` now runs on the bytecode VM byte-identically to native. Higher-order nesting stays native-only. This is not a claim of complete backend parity. `PARITY.tsv` separately ratchets intentionally native-only facilities and known VM gaps, so unsupported behavior cannot masquerade as success.
...
Full engineering detail lives in [CHANGELOG.md](CHANGELOG.md). The user-facing summary and gate matrix are in [RELEASE_NOTES.md](RELEASE_NOTES.md). The complete AD walkthrough is the [Automatic Differentiation guide](docs/guide/AUTOMATIC_DIFFERENTIATION.md), and the memory model is documented in [docs/reference/runtime/memory-model.md](docs/reference/runtime/memory-model.md). Every claim above is tied to shipped code and an executable gate.
**Eshkol v1.3.4-evolve is a resident-correctness release: every defect surfaced by long-duration resident workloads is fixed at the architectural root. Automatic per-iteration memory reclamation now matches explicit `with-region` even for loops that mutate persistent state every tick; `parallel-map` is race-free for closures that allocate and return collections; gradients are exact through every callable form — indirect, wrapped, and curried, with no finite-difference fallback in the gradient path; printed floats round-trip to the identical `double` (R7RS 6.2.6); and the strict type checker accepts idiomatic dynamic-but-validated code via a checked `(the <type> expr)` ascription and predicate-guarded narrowing. The release also lands the high-precision numerics wave — Ozaki-II exact and reduced-precision GEMM tiers, a mixed-precision linear solver, and a native 128-bit integer type — a Moonlab v1.2.0 quantum pin with quantum-natural-gradient support, full hosted-VM tensor-matmul parity, and a linear `Qubit` type.**

Eshkol is R7RS Scheme compiled to native code through LLVM 21, with automatic differentiation as a compiler primitive rather than a tracing library. v1.3.4-evolve is about the property that separates a research demo from something you can leave running as a daemon: correctness that holds over long-duration, resident, and parallel workloads.

- **Automatic memory reclamation matches explicit regions.** This closes the ESH-0214 memory-management series. A resident tick/daemon loop that mutates persistent state on every iteration — a knowledge base, a workspace, a growing list — used to get no per-iteration reclamation at all (the static escape analysis rejected any loop body containing a persistent mutation) and leaked one iteration's transient garbage forever: about 3,366 bytes per tick, roughly 355 MB over 100,000 ticks. Such a loop is now lowered with a per-loop nursery region reusing the same deep-transitive escape-promotion path `with-region` uses: each iteration's allocations land in the nursery, the existing structural write barriers promote any persistent-mutation escapee out at the store, each tail-call back edge promotes the loop-carried out-values and resets the nursery, and the loop exit escapes the result and tears the nursery down — a deterministic generational minor collection with no tracing pause. The same tick loop is now flat at 34 MB, byte-for-byte identical to its explicit `with-region` twin. `with-region` remains available for scratch regions but is no longer required to get flat RSS.
- **Race-free `parallel-map` for collection-valued closures.** A closure mapped in parallel whose body used per-iteration scope reclamation — an internal named-let loop, or a builtin such as `memv` — could return dangling or overlapping structure once the input crossed the parallel threshold. On a pool worker operating on the shared thread-safe arena, scope reclamation now degrades to commit-only (allocations are retained; the shared scope stack is never rewound), so results are identical to serial `map`. ThreadSanitizer reports zero arena data races where it previously reported dozens.
- **Exact gradients through every callable form.** `(gradient f point)` reached through a function parameter or wrapper, and the curried `((gradient f) point)`, are now byte-identical to the direct call for scalar multi-argument, vector, and non-polynomial losses. The operator recovers the callable's arity from its closure metadata instead of assuming a single tensor argument, so a multi-parameter scalar loss is no longer misdispatched. There is no finite-difference fallback anywhere in the gradient path — every form is exact reverse-mode AD.
- **Round-tripping floats.** `display`, `write`, and `number->string` emit the shortest decimal that reads back as the identical `double` (R7RS 6.2.6). Integral doubles keep the no-`.0` form. The native compiler and the bytecode VM share one portable-C routine, so their output is byte-identical. `(sqrt 2.0)` prints `1.4142135623730951`.
- **Strict-mode types that accept idiomatic code.** A checked `(the <type> expr)` ascription asserts a type to the checker as a trusted assertion and is a pure runtime no-op; predicate-guarded narrowing teaches the checker that a value tested by one of eight type predicates is that type inside the guarded branch (honored across `if` and `and`, cancelled at `set!`); sum-type annotations are honored on named-let parameters; and a numeric-tower join gives recursive accumulators their least-upper-bound numeric type. A linear `Qubit` type enforces use-exactly-once on declared linear parameters, giving quantum registers a no-cloning guarantee at the type level.
- **High-precision numerics.** Ozaki-II recovers full-f64 `C = A*B` from reduced-precision tensor cores, certified against an independent CPU f64 reference and against the new native 128-bit integer path; a CUDA INT8 tier and a Metal reduced-precision fully-GPU fast tier are both opt-in and default off. `linear-solve` is a full-f64 dense solver with a mixed-precision iterative-refinement fast path that certifies a full-f64 residual and falls back to a plain-f64 LU when it cannot.
- **Quantum and VM parity.** Moonlab is pinned to v1.2.0, adding `vqe_compute_qgt` (quantum geometric tensor / quantum natural gradient) and a smooth first-principles H2/LiH potential-energy surface; the H2 equilibrium oracle at 0.735 Å is updated to `-1.142200155381` Ha. Hosted-VM tensor matmul parity is complete: `arange` in 1/2/3-argument forms, nested-literal tensor operands, and multi-dimensional `tensor-ref`/`tensor-set!`.

- **Every declared executable surface has evidence, and the evidence is execution-backed.** The language-surface manifest is 1,078/1,078: every one of the 1,078 declared constructs is proven by runtime execution evidence — it dispatched or executed in a passing run — rather than by lexical name-presence, which is now only a diagnostic and earns no release credit. The surface was honestly re-baselined upward from 1,056 as the `i128` tower, `linear-solve`, and the string/pointer conversions landed as new core builtins, and the coverage floor and deficit ledger are ratcheted to 1,078 so the number cannot be walked back silently.
- **VM results are explicit, not overstated.** The parity manifest is 936 rows — 562 `vm-supported`, 45 `native-only-justified`, and 329 `gap`, of which 17 are verified behavioral divergences with reproducible programs. `op:GRADIENT` and `op:DERIVATIVE` move to `vm-supported` this release, so `gradient` runs on the bytecode VM byte-identically to native; higher-order nesting stays native-only. This is not a claim of complete backend parity: `PARITY.tsv` separately ratchets intentionally native-only facilities and known VM gaps so unsupported behavior cannot masquerade as success.
- **Release packages are portable and honestly labelled.** The release payload is 15 platform packages plus `SHA256SUMS.txt`: 16 published files. Linux and Windows x64/ARM64 plus macOS x64/ARM64 cover Lite and XLA; real CUDA packages cover Linux x64/ARM64 and Windows x64. Windows ARM64 CUDA is not advertised, because NVIDIA does not provide the required supported toolkit.

Full engineering detail lives in [CHANGELOG.md](CHANGELOG.md); the user-facing summary and gate matrix are in [RELEASE_NOTES.md](RELEASE_NOTES.md); the complete AD walkthrough is the [Automatic Differentiation guide](docs/guide/AUTOMATIC_DIFFERENTIATION.md); the memory model is documented in [docs/reference/runtime/memory-model.md](docs/reference/runtime/memory-model.md). Every claim above is tied to shipped code and an executable gate.
