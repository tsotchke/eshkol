...
The compiler handles **16,000 nested expressions** through parsing, type checking, and code generation on an 8 MiB stack. Explicit continuations carry each suspended child parse and lowering step, so grammar depth does not consume a native frame per level.

Automatic differentiation composes to arbitrary nesting depth and order on native JIT and AOT. Recursive Taylor levels carry exact coefficients through enclosing derivative passes; unary and complex-valued math functions retain their derivative routes. Dense `matmul`, `tensor-sum`, and `tensor-mean` each record one reverse-mode node, and scalar and dense lowerings produce byte-identical gradients. `tensor-apply` invokes its callable operand through the shared call dispatcher; its four-engine gate checks **31 ordered assertions**.

Exactness follows the point on both engines: `(derivative (lambda (x) (* x x)) 1/3)` evaluates through the exact carrier. It returns `2/3`. Constant-curvature geometry shares one forward and reverse implementation; exact antipode detection is validated by a 4.16-million-case rational sweep.

The VM now reclaims memory in `with-region`. The measured fixture stays at **33 MB, 34 MB, and 34 MB** over 1,000, 4,000, and 16,000 iterations, respectively, compared with **304 MB** when reclamation is disabled. Captured continuations are multi-shot on native JIT, AOT, and the VM; `dynamic-wind` follows re-entry. Browser tensor operations use the same dispatch seam for WebGPU, with CPU fallback when a device or kernel is unavailable.

The language contracts are aligned across engines: `syntax-rules` keeps lexical bindings hygienic; top-level `begin` splices definitions; literals, `read`, and `string->number` share the R7RS number grammar, including complex forms and prefixes. Wrong arity raises a catchable error, caught conditions remain silent, record and container accessors check their operands, standard ports are parameters on the VM, and VM hash tables use `equal?` keys. The browser REPL returns complete answers.

The release record reports **388/388** VM parity differential checks. The manifest classifies **962** rows: 620 VM supported, 46 native only with justification, and 296 gaps. The declared language surface contains **1,116** constructs. The exact CTest total is pending a full run at the tagged commit and is not reported as a release figure.

Gabriel Kahen contributed the ESKM v1 compatibility and cross-reader work, the parameter-binding order correction, and the allocation-failpoint approach for region promotion. LJGz corrected the list tutorial's `take`, `drop`, and `sort` argument order (#694).

Read the [release notes](RELEASE_NOTES.md) for mechanisms and verification, [upgrading guidance](docs/UPGRADING.md) for behavior changes, and [known issues](docs/KNOWN_ISSUES.md) for current limits.
...
- **The gates, measured on the release cut.** The aggregate suite is 45/45 suites and 770 individual tests. CTest is completion-oracle evidence. The SICP full-book gate is 88/88 probes across all five chapters under both `-r` and AOT. The reference-Scheme differential oracle is 34/34 AGREE against chibi-scheme 0.12.0. The VM parity differential runs as a release gate. The qLLM oracle gate is 10/10. ICC readiness reports 100, verdict `ready`.
