...
- **nested_ellip** WRONG from depth 1 (got=0 want=1) [aot+r] — nested ellipsis `x ... ...` unsupported; macro-expansion error is non-fatal so the program exits 0 with a silently-wrong value (ESH-0128).
- **qq_macro** WRONG from depth 2 (got=1 want=2) [aot+r] — pattern variable inside a quasiquote-unquote in a macro template is not substituted; d1 passes only coincidentally (ESH-0127).

## Related parser finding (not a depth family — worked around in the generator)
- **ESH-0126** a `syntax-rules` rule whose whole template is quote/quasiquote *shorthand* (`'()`, `` `x ``) is rejected ("expected closing paren after macro rule template"); the `(quote ...)` long form and nested shorthand (`(cons x '())`) both work.
